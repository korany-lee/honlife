// KM-1483 Amplitude
function trackingCode(type, _action_data){
    var _event_info = _action_data.secondary_category_id;
    if(_event_info === null){
        _event_info = _action_data.primary_category_id;
    }
    KurlyTracker.setEventInfo(_event_info);
    KurlyTracker.setAction('select_category', _action_data).sendData();
    if(type === 'pc'){
...
Vue.component('gnb-menu',{
    ...
    ,methods:{
        ...
        ,clickEvent:function(data, type, sub){
            ...
            sessionStorage.gListCategoryNo = no;
            sessionStorage.gListScrolltop = 0;
            sessionStorage.gListCheckPageNo = 1;
            ...
            trackingCode('mobile', _action_data);
...
Vue.component('gnb-recommend',{
    ...
    <li>\
        <a class="link sub" @click="clickEvent(recommendMenu, type)">\
            <span class="thumb" :style="{ \'background-image\': \'url(\' + recommendMenu.thumbnail_url + \')\'}">\
                <img src="https://res.kurly.com/pc/img/1810/bg_blink_236x179.png" alt="" v-if="type === \'pc\'">\
                <img src="https://res.kurly.com/mobile/img/1908/bg_blink_163x120.png" alt="" v-if="type === \'mobile\'">\
            </span>\
    ...
    ,methods:{
        clickEvent:function(item, type){
            ...
            trackingCode(type, _action_data);
            // END : KM-1483 Amplitude
...
var gnbMenu = new Vue({
    ...
    methods: {
        update:function(){
            ...
            .then(function(response) {
                ...
                for(var i = 0 ; i < this.getData.categories.length ; i++ ){
                    ...
                    this.$set(this.dataGnb[i],"subOpen",false);
                }
        ...
        ,openAction : function(result, idx){
            this.dataGnb[idx].subOpen = result;
            for(var i = 0 ; i < this.dataGnb.length ; i++ ){
                if(idx !== i){
                    this.dataGnb[i].subOpen = false;
                }
            }
        }
